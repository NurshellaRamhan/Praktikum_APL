...
void tambahLaundry() {
    if (banyakLaundry < MAX_LAUNDRY) {
        ...
        newLaundry.statusPengerjaan = false;        // status pengerjaan awal
        newLaundry.statusPembayaran = false;    // status pembayaran awal
        ...
        for (int i = 0; i < jumlahPakaian; ++i) {
            ...
            cout << "Jenis pakaian (formal)/(nonformal): " << (i + 1) << ": ";
            cin.ignore();
            getline(cin, detail.jenisPakaian);
            cout << "Berat pakaian " << (i + 1) << " (kg): ";
            cin >> detail.beratPakaian;
            cout << "Harga satuan pakaian " << (i + 1) << ": ";
            cin >> detail.hargaSatuan;
            newLaundry.detailLaundry.push_back(detail);
        }
...
void updateLaundry(const string& nama) {
    string updateNama;
